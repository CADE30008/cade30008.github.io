...
A controller \(C(s)\) acts on the error \(e = \theta_\mathrm{ref} - \theta\), and the plant \(G(s)\) turns the elevator command \(u\) into pitch attitude \(\theta\). A disturbance \(d\) adds to the elevator command. It stands for anything that pitches the aircraft without the controller asking, such as a trim change after a centre-of-gravity shift. Everything about the closed loop follows from the **loop gain**
...
- **The crossover frequency \(\omega_\mathrm{c}\)**, where \(|L(j\omega_\mathrm{c})| = 1\), separates the two. It sets how fast the closed loop is.
- **The phase margin**, \(PM = 180^\circ + \angle L(j\omega_\mathrm{c})\), sets how gracefully the loop hands over at crossover. It determines the damping and the overshoot.
...
With \(C(s) = K_\mathrm{p}\), the loop is \(L = K_\mathrm{p}G\). Changing \(K_\mathrm{p}\) slides the magnitude plot up or down and leaves the phase plot alone. That single fact explains everything proportional control can and cannot do.
...
so with \(K_\mathrm{p} = 1\) the gain margin is 6, or 15.6 dB. Any \(K_\mathrm{p} > 6\) makes the loop unstable.

**Phase margin.** With \(K_\mathrm{p} = 1\) the crossover frequency is 1.56 rad/s and the phase there is −136.8°, a phase margin of 43.2°.
...
<figure markdown="span">
  ...
  <figcaption>Margins of the pitch loop with \(K_\mathrm{p} = 1\).</figcaption>
</figure>
...
With \(K_\mathrm{p} = 1\), a 43° phase margin suggests \(\zeta \approx 0.43\) and 22% overshoot. The simulation gives 25%. Retuning for a 60° phase margin needs \(K_\mathrm{p} = 0.509\) and predicts 9.5% overshoot, against 8.1% simulated.

**Crossover frequency sets speed.** The closed-loop bandwidth is close to \(\omega_\mathrm{c}\), and the rise time scales as \(1/\omega_\mathrm{c}\). Halving the crossover frequency roughly doubles the rise time. The 60° design crosses over at 0.92 rad/s instead of 1.56, and its rise time grows from 0.76 s to 1.37 s.
...
The final value theorem gives the error once transients have died away. For the error \(e = \theta_\mathrm{ref} - \theta\),

$$
e_\mathrm{ss} = \lim_{s\to 0} s\,E(s).
$$
...
| Ramp \(\theta_\mathrm{ref} = \Omega t\), a steady pitch-over | \(\Omega / K_\mathrm{v}\), with \(K_\mathrm{v} = \lim_{s\to0} sL(s) = 2K_p\) | 0 |
...
A trim change equivalent to 2° of elevator leaves a 2° attitude error when \(K_\mathrm{p} = 1\). Only an integrator *before* the disturbance, in the controller, drives this error to zero.
...
which adds a zero at \(s = -1/T_\mathrm{d}\). Above \(1/T_\mathrm{d}\) the magnitude rises at 20 dB per decade and the phase gains up to 90°. The phase lead at crossover is
...
With \(N = 10\) the filter caps the lead at about 56°, and the high-frequency gain at \(K_\mathrm{p}(N+1) = 11K_p\). Values of \(N\) between 8 and 20 are typical.

**Designing the PD controller.** We aim for crossover at 3 rad/s, about twice as fast as \(K_\mathrm{p} = 1\), with a 60° phase margin.
...
3. Ideal PD needs \(\tan^{-1}(3T_d) = 43^\circ\), so \(T_\mathrm{d} = 0.311\) s. The filter takes back a few degrees, so the filtered design needs \(T_\mathrm{d} = 0.349\) s.
4. Choose \(K_\mathrm{p}\) so that \(|L(j3)| = 1\). That gives \(K_\mathrm{p} = 1.86\), and \(K_\mathrm{d} = K_\mathrm{p}T_d = 0.649\).
...
<figure markdown="span">
  ...
  <figcaption>PD achieves the speed of \(K_\mathrm{p} = 1\) with better damping than the 60° proportional design.</figcaption>
</figure>

| Design | \(\omega_\mathrm{c}\) (rad/s) | PM | Rise time | Overshoot | 2% settling | Offset per unit \(d\) |
|---|---|---|---|---|---|---|
| P, \(K_\mathrm{p} = 0.509\) | 0.92 | 60.0° | 1.37 s | 8.1% | 4.34 s | 1.97 |
| PD, \(K_\mathrm{p} = 1.86\), \(K_\mathrm{d} = 0.649\) | 3.00 | 60.1° | 0.79 s | 0.5% | 1.31 s | 0.54 |

At the same phase margin, PD rises 1.7 times faster and settles 3.3 times faster. It also has a higher \(K_\mathrm{p}\), which cuts the disturbance offset to about a quarter.
...
$$
u = K_\mathrm{p}\,e + K_\mathrm{i}\!\int e\,dt - K_\mathrm{d}\,\frac{d\theta}{dt}.
$$
...
It adds an integrator and a zero at \(s = -1/T_\mathrm{i}\). Below \(1/T_\mathrm{i}\) the loop gain rises without limit as frequency falls, so the loop gains a type. Constant disturbances now give zero steady-state error. The price is phase lag at crossover:

$$
\phi_{lag} = \tan^{-1}\!\left(\frac{1}{\omega_\mathrm{c} T_\mathrm{i}}\right).
$$

Placing the zero a factor of 5 to 10 below crossover keeps this to between 6° and 11°. Here \(T_\mathrm{i} = 3\) s and \(\omega_\mathrm{c} = 3\) rad/s, so the lag is \(\tan^{-1}(1/9) = 6.3^\circ\).
...
The disturbance offset is now removed, as the disturbance figure above shows. There is a cost in the reference response, though. The closed loop has a slow pole near the PI zero at \(-1/T_\mathrm{i}\), which gives a slow final approach. Overshoot also rises to 18.7%, because the reference passes through the PI zero. The 2% settling time lengthens from 1.3 s to 6.9 s, even though the rise time improves.
...
A larger \(T_\mathrm{i}\) shortens the overshoot and lengthens the time taken to remove a disturbance. If the reference response matters more, the standard fixes are to weight the reference in the proportional term or to add a prefilter. Both reshape the reference response without touching the loop.
...
3. **Choose \(T_\mathrm{i}\)** so that \(1/T_\mathrm{i}\) is 5 to 10 times below crossover, and note the phase it costs.
4. **Find the lead needed** at crossover: \(\phi_{lead} = PM - 180^\circ - \angle G(j\omega_\mathrm{c}) + \phi_{lag}\). Solve for \(T_\mathrm{d}\), with a filter of \(N\) between 8 and 20.
5. **Set \(K_\mathrm{p}\)** so that \(|L(j\omega_\mathrm{c})| = 1\).
6. **Check** the gain margin, the peak actuator demand for a typical step, the noise gain \(K_\mathrm{p}(N+1)\), and the step and disturbance responses. Then iterate.

| Design | \(K_\mathrm{p}\) | \(K_\mathrm{i}\) | \(K_\mathrm{d}\) | \(\omega_\mathrm{c}\) | PM | GM | Overshoot | Peak \(\delta_e\), 10° step | Offset per unit \(d\) |
|---|---|---|---|---|---|---|---|---|---|
...
- Set \(K_\mathrm{i} = K_\mathrm{d} = 0\) and raise \(K_\mathrm{p}\) until the gain margin reaches zero. Check that this happens at \(K_\mathrm{p} = 6\).
...
|---|---|---|---|
| P, \(K_\mathrm{p}\) | Slides the magnitude up or down; phase unchanged. | Faster, but less damped as the gain rises. | Can't give speed and damping together. |
| I, \(K_\mathrm{i} = K_\mathrm{p}/T_\mathrm{i}\) | Unlimited gain at low frequency; lag near \(1/T_\mathrm{i}\). | Removes offsets from constant disturbances. | Phase lag at crossover, a slower tail, windup. |
| D, \(K_\mathrm{d} = K_\mathrm{p}T_d\) | Phase lead around \(1/T_\mathrm{d}\). | Allows a higher crossover with the same damping. | Noise gain \(K_\mathrm{p}(N+1)\); kick unless it acts on the measurement. |
