...
description: "Worked solutions to the Lecture 3 example sheet."
lesson: l03-pid-control
---
...
[Handout](index.md)
[Slides](../slides/l03-pid-control/index.html)
</div>
...
2. Over a common denominator,
    ...
    since \(T_iT_d = 1\). The zeros are the roots of \(s^2 + 4s + 1 = 0\), which are \(s = -2 \pm \sqrt{3}\). That is \(s = -0.268\) and \(s = -3.732\).

3. The filter pole is at \(s = -N/T_d = -40\) rad/s. At high frequency the derivative term tends to \(K_d/T_f = K_cN\). The total gain is therefore \(K_c(N + 1) = 22\).
...
2. The velocity constant is
    ...
    The ramp error is therefore \(\Omega/K_v = 2/3 = 0.67^\circ\). The attitude lags the command by about two-thirds of a degree.

3. The disturbance enters before the plant's integrator:

    $$
    \theta_{ss} = \lim_{s\to 0}\frac{G(s)}{1 + K_pG(s)}\,d = \frac{d}{K_p} = \frac{1.5^\circ}{1.5} = 1.0^\circ,
    $$
...
2. A 45° phase margin needs \(\tan^{-1}(\omega/3) + \tan^{-1}(\omega/15) = 45^\circ\). Using the tangent identity,
    ...
    so \(\omega_c = 2.23\) rad/s. Then

    $$
    K_p = \frac{\omega_c\sqrt{\omega_c^2 + 9}\,\sqrt{\omega_c^2 + 225}}{81} = 1.56.
    $$

    The gain margin falls to \(10/1.56 = 6.4\), or 16.2 dB.

3. With \(\zeta \approx 0.45\), the rule of thumb predicts \(M_p = e^{-\pi\zeta/\sqrt{1-\zeta^2}} = 20.5\%\). Simulation gives 23.3%.

4. A 60° margin needs the two lags to sum to 30°:

    $$
    \frac{0.4\,\omega}{1 - \omega^2/45} = \tan 30^\circ = 0.577 \quad\Rightarrow\quad \omega_c = 1.38\ \text{rad/s},
    $$

    and \(K_p = 0.849\), about 45% less gain. Rise time scales as \(1/\omega_c\), so the response should be about \(2.23/1.38 = 1.61\) times slower. Simulation gives rise times of 0.54 s and 0.92 s, a ratio of 1.70. The overshoot falls to 8.1%.
...
2. At 4 rad/s the plant phase is \(-180^\circ - \tan^{-1}(0.2) = -191.3^\circ\). The controller must supply \(45^\circ + 11.3^\circ = 56.3^\circ\) of lead. Conveniently,
    ...
    so \(4T_d = 1.5\) and \(T_d = 0.375\) s. The plant gain at 4 rad/s is

    $$
    |G_z(j4)| = \frac{1}{1.5 \times 16 \times \sqrt{1 + 0.04}} = 0.0409,
    $$

    and \(|1 + j1.5| = 1.80\). Setting \(|L(j4)| = 1\) gives \(K_p = 13.6\) N/m and \(K_d = K_pT_d = 5.09\) N s/m.
...
4. The integral term costs \(\tan^{-1}\!\big(1/(4 \times 2.5)\big) = 5.7^\circ\) at 4 rad/s. For the ideal PID,
    ...
    With \(T_d = 0.4\) s this is \(1 + j1.5\), exactly as for the PD design. Both phase and magnitude at 4 rad/s are unchanged, so the phase margin is 45° with the same \(K_p = 13.6\) N/m. That gives \(K_i = K_p/T_i = 5.43\) N/(m s) and \(K_d = 5.43\) N s/m. The altitude offset is now removed.
...
1. With D on the error, the first-instant demand is \(K_p(N + 1)\,\Delta\theta_{ref} = 2.03 \times 11 \times 5^\circ = 112^\circ\). The elevator slams to its 20° stop. With D on the measurement only the proportional term reacts, giving \(K_p\,\Delta\theta_{ref} = 10.2^\circ\).
2. At high frequency the filtered derivative acting on attitude has gain \(K_pN = 20.3\), and the whole controller \(K_p(N+1) = 22.3\). So 0.1° of attitude noise can produce up to about 2.2° of elevator jitter. Using the gyro, the derivative path multiplies pitch rate by \(K_d = 0.661\) s. So 0.5°/s of gyro noise gives only about 0.33° of elevator jitter.
...
=== "Python"
    ...
    ```python
    --8<-- "l03-pid-control/code/q6_solution.py"
    ```

=== "MATLAB"

    ```matlab
    --8<-- "l03-pid-control/code/q6_solution.m"
    ```
