...
Both of those hold for **unity feedback**, where the output is fed
straight back and compared with the reference. Put a sensor \( H(s) \) in
the return path and the closed loop is \( T = \dfrac{G}{1 + GH} \), whose
numerator and denominator both pick up parts of \( H \). If there is a
controller in the forward path, the shortcut still works provided you
apply it to the whole forward path, \( CG \), rather than to the plant
alone.
